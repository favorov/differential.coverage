...
if(! 'raw_differtial_coverage_gencode_26_gff' %in% ls()){
  raw_differtial_coverage_gencode_26_gff=
...
gencode26_genes<-
  GRanges(as.data.frame(raw_differtial_coverage_gencode_26_gff) %>% 
      filter(type=="gene") %>% 
...
gencode26_genes$ensembl<-sapply(strsplit(gencode26_genes$ensembl,'.',fixed = TRUE),"[[",1)

#we are in the package root folder
usethis::use_data(gencode26_genes,overwrite = TRUE)
