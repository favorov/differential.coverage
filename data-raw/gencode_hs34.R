...
if(! 'raw_differtial_coverage_gencode_hs34_gff' %in% ls()){
  raw_differtial_coverage_gencode_hs34_gff=
      readGFF("http://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_34/gencode.v34.annotation.gff3.gz",
              version=3)
}
...
gencode_hs34_genes<-
  GRanges(as.data.frame(raw_differtial_coverage_gencode_hs34_gff) %>% 
      filter(type=="gene") %>% 
...
gencode_hs34_genes$ensembl<-sapply(strsplit(gencode_hs34_genes$ensembl,'.',fixed = TRUE),"[[",1)

#we are in the package root folder
usethis::use_data(gencode_hs34_genes,overwrite = TRUE)
